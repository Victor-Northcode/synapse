...
2. **Подпись.** Настроена: upload-keystore в `game/signing/`,
...
см. `lib/core/interstitial_gate.dart`); баннеров и App Open нет,
внутриигровых покупок нет — осознанные решения владельца.
...
- Реклама только от **AdMob** — сеть сертифицирована для Families Ads
  Program; форматы — rewarded по явному нажатию и interstitial между
  уровнями (после победы, с частотным капом и капом на сессию).
...
- Тип: *Идентификаторы устройства* — собирает партнёр (Google AdMob)
  для рекламы и защиты от мошенничества; передаются Google; шифруются
  при передаче; удаление — сбросом рекламного идентификатора в
  настройках системы.
...
Games/Puzzle + возрастной экран).

**App Store → App Privacy** (ASC требует заполнить, раз в бинаре есть
NSUserTrackingUsageDescription; по таблице
developers.google.com/admob/ios/data-disclosure, сборщик — AdMob,
трекинг происходит только у взрослых после согласия ATT):
- «Do you collect data?» — Да.
- **Data Used to Track You**: Identifiers → *Device ID*;
  Usage Data → *Advertising Data*.
- **Data Not Linked to You** (цели: Third-Party Advertising,
  Analytics, App Functionality): Location → *Coarse Location* (оценка
  по IP); Identifiers → *Device ID*; Usage Data → *Advertising Data*,
  *Product Interaction*; Diagnostics → *Crash Data*, *Performance
  Data*.
- «Linked to You» — ничего: аккаунтов нет, игровой псевдоним случайный.
- В заметке для ревьюеров пояснить HTTP-исключение для dreamlo.com:
  передаётся только анонимный игровой счёт со случайным псевдонимом,
